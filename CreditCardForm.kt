...
class CreditCardForm(private val cardInput: EditText) {
    ...
    fun defaults() = apply {
        ...
        CardType() // Создаем новую карту
                .name(AMEX) // Задаем ей УНИКАЛЬНОЕ ИМЯ
                .cvsLength(4) // Указываем сколько символов CVS используется для этого типа
                .pattern("^3[47][0-9]{2}$") // Паттерн для определения типа карты
                .addRule(4, 6, 5) // Сколько элементов в каждой группе должно быть.
    }
